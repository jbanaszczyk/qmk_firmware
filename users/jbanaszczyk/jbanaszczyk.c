...
layer_state_t layer_state_set_user(layer_state_t state) {
    state = layer_state_set_alt_layers(state);

    dprintf("____ layer_state_set_user %d\n", state);
    dprintf("Layer:  %08lX(%u) %08lX(%u)\n", layer_state, get_highest_layer(layer_state),state, get_highest_layer(state));
